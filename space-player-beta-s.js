...
class SpacePlayer {
    static ICONS = {
        ...
        subtitle: '<svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-8 12H4v-2h8v2zm8 0h-6v-2h6v2zm0-4H4V8h16v4z"/></svg>'
    };
    ...
    render() {
        ...
        style.textContent = `
            ...
            .control-btn { background: none; border: none; cursor: pointer; display: flex; align-items: center; padding: 8px; border-radius: 50%; transition: 0.2s; position: relative; }
            .control-btn:hover { background: rgba(255,255,255,0.1); transform: scale(1.1); }
            .control-btn svg { width: 26px; height: 26px; fill: #fff; }
            .control-btn.active::after { content: ''; position: absolute; bottom: 4px; left: 50%; transform: translateX(-50%); width: 12px; height: 2px; background: #ff0000; border-radius: 2px; }
            ...
            .context-item.active, .setting-item.active { color: #ff0000; font-weight: 600; }
        ...
        this.container.innerHTML = `
            ...
            <video class="space-video" src="${this.options.src}" poster="${this.options.poster}" playsinline crossorigin="anonymous">
                ${this.options.subtitle ? `<track kind="subtitles" src="${this.options.subtitle}" srclang="ko" label="한국어" default>` : ''}
            </video>
            ...
            <div class="space-controls-overlay">
                ...
                <div class="space-controls-main">
                    ...
                    <div class="space-right-controls">
                        <button class="control-btn subtitle-btn active">${SpacePlayer.ICONS.subtitle}</button>
            ...
            <div class="settings-panel">
                <div class="settings-wrapper">
                    <div class="settings-column main-menu-column">
                        ...
                        <div class="setting-item" data-sub="subSub"><span>자막 설정</span><span class="val cur-sub">표준 ${SpacePlayer.ICONS.arrow}</span></div>
                    ...
                    <div class="settings-column">
                        ...
                        <div class="submenu-container sub-sub" id="subSub">
                            <div class="settings-header back-btn">${SpacePlayer.ICONS.back} 자막 크기</div>
                            <div class="setting-item opt-sub-size" data-v="14px">작게</div>
                            <div class="setting-item opt-sub-size active" data-v="18px">보통</div>
                            <div class="setting-item opt-sub-size" data-v="24px">크게</div>
                            <div class="setting-item opt-sub-size" data-v="32px">매우 크게</div>
                        </div>
    ...
    bindEvents() {
        ...
        // 자막 토글 버튼
        dom.subtitleBtn.onclick = e => {
            e.stopPropagation();
            const track = dom.video.textTracks[0];
            if (track) {
                const isShowing = track.mode === 'showing';
                track.mode = isShowing ? 'disabled' : 'showing';
                dom.subtitleBtn.classList.toggle('active', !isShowing);
            }
        };
        ...
        this.container.querySelectorAll('.opt-sub-size').forEach(opt => {
            opt.onclick = () => {
                const size = opt.dataset.v;
                // 스타일 시트 수정을 통해 모든 자막에 적용
                const sheet = document.styleSheets[0];
                const rule = `.space-video::cue { font-size: ${size} !important; }`;
                sheet.insertRule(rule, sheet.cssRules.length);
                
                this.container.querySelectorAll('.opt-sub-size').forEach(el => el.classList.remove('active'));
                opt.classList.add('active');
                this.container.querySelector('.cur-sub').innerHTML = opt.textContent + SpacePlayer.ICONS.arrow;
                this.container.querySelector('.back-btn').click();
            };
        });
